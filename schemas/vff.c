...
G_MODULE_EXPORT gchar* schema_id = "vff";
...
void schema_iteration() {
   gdouble resul_deg;
   resul_deg = atan2(target_y, target_x) * RADTODEG;
...
G_MODULE_EXPORT void schema_startup (void) {
   ...
   if (!loaded) {
      putSchemaVar((gpointer)&vff_x, "vff_x", schema_id);
      putSchemaVar((gpointer)&vff_y, "vff_y", schema_id);
